...
public class Utils {
  ...
  public static float maxAbs(PointF3D point) {
    //TODO not sure if this does the right thing changed the max of the primitives to this
    return Float.max(Float.max(Math.abs(point.getX()), Math.abs(point.getY())), Math.abs(point.getZ()));
  }
